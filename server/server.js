...
// Render (and most PaaS hosts) sit behind a proxy — trusting it keeps
// req.ip / protocol detection correct.
...
// Explicit admin routes, registered BEFORE express.static. This guarantees
// /admin and /admin/ always load the admin console — some hosts/CDNs only
// serve a directory's index.html for URLs that already end in a slash, so
// relying only on express.static's default directory handling can end up
// falling through to the main site instead.
...
// ---------- 404 handling ----------
// Unknown API routes get a proper JSON 404 instead of falling through to HTML.
app.use('/api', (req, res) => {
  res.status(404).json({ error: 'Not found' });
});

// Any other unmatched route (e.g. a typo'd URL) gets a plain 404, rather than
// silently serving the homepage — that's what previously made a broken/typo'd
// admin URL look like it was "showing the website" instead of a clear error.
app.use((req, res) => {
